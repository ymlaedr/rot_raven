defmodule RotRavenWeb.GpsRecord do
  ...
  def render(assigns) do
    ~L"""
      ...
      <tr>
        <td><%= Map.from_struct(record)[:timestamp] %></td>
        <td><%= Map.from_struct(record)[:latitude] %></td>
        <td><%= Map.from_struct(record)[:longitude] %></td>
        <td><%= Map.from_struct(record)[:altitude] %></td>
        <td><%= Map.from_struct(record)[:accuracy] %></td>
        <td><%= Map.from_struct(record)[:altitude_accuracy] %></td>
        <td><%= Map.from_struct(record)[:heading] %></td>
        <td><%= Map.from_struct(record)[:speed] %></td>
        <td><%= Map.from_struct(record)[:speed_measurement] %></td>
      </tr>
      <% end %>
    </table>
  ...
  def handle_info({:submit, records_json}, socket) do
    ...
    records_json
    |> Poison.decode!()
    |> Enum.each(&(%GpsRecord{} |> GpsRecord.changeset(&1) |> IO.inspect()))
